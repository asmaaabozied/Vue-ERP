<template>
  <div>
    <top-bar-card
        ...
        :submit="updateGuarantee"
        :id="formData.id"
        :editUrlName="`edit-guarantees`"
        :navigationUrl="`guarantees`"
        :model="`guarantee`"
        :mainModuel="mainModuel"
        :MainCategory="`cards`"
        :pageName="$t(`Guarantees`)"
        :operation="`edit`"
        :listUrlName="`guarantees`"
    ...
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updateGuarantee" id="currentForm">
...
export default {
  ...
  methods: {
    async updateGuarantee() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("updateGuarantee", params);
        } catch (errors) {
    ...
    async getGuarantee() {
      const getGuarantee = await this.$store.dispatch("getGuarantee", {
        Id: this.$route.params.id,
      });
      this.formData = getGuarantee.data.data;
      console.log(typeof this.formData.code);
    },
