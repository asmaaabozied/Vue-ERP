<template>
  <div>
    <top-bar-card
      ...
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`nav.car priceList`"
      :operation="`list`"
      :delete_action="`deletePriceList`"
      :ids="this.ids"
    />
...
export default {
  ...
  computed: {
    ...
    priceLists() {
      this.rows = this.$store.getters.getPriceLists.data;
  ...
  methods: {
    ...
    async getResults() {
      console.log('ahmed');
      await this.$store.dispatch("fetchPriceLists");
      this.rows = this.$store.getters.getPriceLists.data;
    },
    ...
    async Delete() {
      ...
      if (result.value) {
        await this.$store.dispatch("deletePriceList", params);
