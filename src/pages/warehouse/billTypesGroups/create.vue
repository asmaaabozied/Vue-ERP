<template>
  <div>
    <top-bar-card
      :newUrlName="`add-billTypesGroups`"
      :submit="newBillTypeGroup"
      :id="0"
      :editUrlName="`edit-billTypesGroups`"
      :navigationUrl="`BillTypeGroupNavigation`"
      :model="`billTypeGroup`"
      :mainModuel="mainModuel"
      :MainCategory="MainCategory"
      :pageName="`billTypesGroups`"
      :operation="`add`"
      :listUrlName="`billTypesGroups`"
    />
    ...
    >
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="newBillTypeGroup" id="BillTypeGroup">
...
export default {
  ...
  methods: {
    async newBillTypeGroup() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("storeBillTypeGroup", {
            data: this.formData,
          });
        } catch (errors) {
    ...
    async getCode() {
      const code = await this.$store.dispatch("getBillTypeGroupCode");
