<template>
  <div>
    <top-bar-card
      :newUrlName="`edit-tags`"
      :submit="updateTag"
      :editUrlName="`edit-tags`"
      :navigationUrl="`tags`"
      :model="`tag`"
      :listUrlName="`tags`"
      :mainModuel="`tools`"
    ...
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updateTag" id="tag">
...
export default {
  ...
  methods: {
    async updateTag() {
      console.log("Hit update Tag method");
      this.$v.$touch();
      if (this.$v.$invalid) {
        console.log(this.$v);
        ...
        try {
          ...
          await this.$store.dispatch("updateTag", params);
        } catch (errors) {
    ...
    async getTag() {
      const getTag = await this.$store.dispatch("getTag", {
        Id: this.$route.params.id,
      });
      this.formData = getTag.data.data;
    },
