<template>
  <div>
    <top-bar-card
        ...
        :mainModuel="`tools`"
        :MainCategory="`cards`"
        :pageName="`nav.customers`"
        :operation="`list`"
        :delete_action="`deleteCustomer`"
        :ids="this.ids"
    />
...
export default {
  ...
  computed: {
    ...
    customers() {
      this.rows = this.$store.getters.getCustomer.data;
  ...
  methods: {
    ...
    async Delete(ids) {
      ...
      if (result.value) {
        await this.$store.dispatch("deleteCustomer", params);
