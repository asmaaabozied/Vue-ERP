<template>
  <div>
    <top-bar-card
      ...
      :newUrlName="`add-customers`"
      :submit="updateCustomer"
      :id="formData.id"
      :editUrlName="`edit-customers`"
      :navigationUrl="`customersNavigation`"
      :model="`Customer`"
      :listUrlName="`customers`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`customers`"
      :operation="`edit`"
    ...
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updateCustomer" id="Customer">
...
<!--                    </span>-->
                    ...
                    <button
                        @click.prevent="clearValueInput(`name_ar`)"
                        id="button_ar"
                      class="close d-none"
                      ref="button_ar"
                    >
...
<!--                    </span>-->
                    ...
                    <button
                        @click.prevent="clearValueInput(`name_en`)"
                      id="button_en"
                      class="close d-none"
                      ref="button_en"
                    >
              ...
              >
                <div class="form-group">
                  <input
                      ...
                      class="form-control w-100"
                      :class="{
                      'is-invalid': $v.formData.tax_number.$error || backErrors [`tax_number`],
                      'is-vaild': !$v.formData.tax_number.$invalid,
                    }"
                      id="tax"
                      ref="tax"
                      v-model="$v.formData.tax_number.$model"
                      :placeholder="`tax_number`"
                      @keyup="chanagClass($event, 'tax_number')"
                  />
...
<!--                  <span-->
<!--                      class="text-danger"-->
<!--                      v-if="backErrors[`tax_number`]"-->
<!--                      :class="seenName"-->
<!--                  >-->
<!--                    {{ backErrors[`tax_number`][0] }}-->
<!--                  </span>-->

                  <button
                      @click.prevent="clearValueInput(`tax_number`)"
                      id="button_tax_number"
                      class="close d-none"
                      ref="button_tax_number"
                  >
                    &times;
                  </button>
                </div>
              ...
              >
                <div class="form-group">
                  <input
                      ...
                      class="form-control w-100"
                      :class="{
                      'is-invalid': $v.formData.fax_number.$error || backErrors [`fax_number`],
                      'is-vaild': !$v.formData.fax_number.$invalid,
                    }"
                      id="fax"
                      ref="fax"
                      v-model="$v.formData.fax_number.$model"
                      :placeholder="`fax_number`"
                      @keyup="chanagClass($event, 'fax_number')"
                  />
...
<!--                  <span-->
<!--                      class="text-danger"-->
<!--                      v-if="backErrors[`fax_number`]"-->
<!--                      :class="seenName"-->
<!--                  >-->
<!--                    {{ backErrors[`fax_number`][0] }}-->
<!--                  </span>-->

                  <button
                      @click.prevent="clearValueInput(`fax_number`)"
                      id="button_fax_number"
                      class="close d-none"
                      ref="button_fax_number"
                  >
                    &times;
                  </button>
                </div>
              ...
              >
                <div class="form-group">
                  <input
                      ...
                      class="form-control w-100"
                      :class="{
                      'is-invalid': $v.formData.address.$error || backErrors [`address`],
                      'is-vaild': !$v.formData.address.$invalid,
                    }"
                      id="address"
                      ref="address"
...
<!--                  </span>-->
                  ...
                  <button
                      @click.prevent="clearValueInput(`address`)"
                      id="button_address"
                      class="close d-none"
                      ref="button_address"
                  >
              ...
              >
                <div class="form-group">
                  <input
                      ...
                      class="form-control w-100"
                      :class="{
                      'is-invalid': $v.formData.telephone.$error || backErrors [`telephone`],
                      'is-vaild': !$v.formData.telephone.$invalid,
                    }"
                      id="telephone"
                      ref="telephone"
...
<!--                  <span-->
<!--                      class="text-danger"-->
<!--                      v-if="backErrors[`telephone`]"-->
<!--                      :class="seenName"-->
<!--                  >-->
<!--                    {{ backErrors[`telephone`][0] }}-->
<!--                  </span>-->

                  <button
                      @click.prevent="clearValueInput(`telephone`)"
                      id="button_telephone"
                      class="close d-none"
                      ref="button_telephone"
                  >
                    &times;
                  </button>
                </div>
...
export default {
  ...
  methods: {
    async updateCustomer() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("updateCustomer", params);
        } catch (errors) {
    ...
    async getCustomer() {
      const getCustomer = await this.$store.dispatch("getCustomer", {
        Id: this.$route.params.id,
      });
      this.formData = getCustomer.data.data;
    },
    async getBranches() {
