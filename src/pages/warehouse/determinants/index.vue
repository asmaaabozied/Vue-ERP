<template>
  <div>
    <top-bar-card
      ...
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`nav.car determinant`"
      :operation="`list`"
      :delete_action="`deleteDeterminant`"
      :ids="this.ids"
    />
...
export default {
  ...
  computed: {
    ...
    determinants() {
      this.rows = this.$store.getters.getDeterminants.data;
      return this.$store.getters.getDeterminants;
    },
    visibleFields() {
  ...
  methods: {
    ...
    async getResults() {
      console.log('ahmed');
      await this.$store.dispatch("fetchDeterminants");
      this.rows = this.$store.getters.getDeterminants.data;
    },
    ...
    async Delete() {
      ...
      if (result.value) {
        await this.$store.dispatch("deleteDeterminant", params);
