<template>
  <div>
    <top-bar-card
      :listUrlName="`trailerWheels`"
      :newUrlName="`add-trailerWheels`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`nav.trailer Wheels`"
      :operation="`list`"
      :delete_action="`deleteTrailerWheels`"
      :ids="this.ids"
    />
...
export default {
  ...
  methods: {
    ...
    async getResults() {
      await this.$store.dispatch("fetchtrailerWheels");
      this.rows = this.$store.getters.gettrailerWheels.data;
    },
     ...
     async Delete() {
      ...
      if (result.value) {
        await this.$store.dispatch("deleteTrailerWheels", params);
