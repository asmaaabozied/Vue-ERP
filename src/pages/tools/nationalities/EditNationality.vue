<template>
  <div>
    <top-bar-card
        ...
        :submit="updateNationality"
        :id="formData.id"
        :editUrlName="`edit-nationalities`"
        :navigationUrl="`nationalities`"
        :model="`nationality`"
        :mainModuel="mainModuel"
        :MainCategory="`cards`"
        :pageName="$t(`nationalities`)"
        :operation="`edit`"
    ...
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updateNationality" id="currentForm">
...
export default {
  ...
  methods: {
    async updateNationality() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("updateNationality", params);
        } catch (errors) {
    ...
    async getNationality() {
      const getNationality = await this.$store.dispatch("getNationality", {
        Id: this.$route.params.id,
      });
      this.formData = getNationality.data.data;
      console.log(typeof this.formData.code);
    },
