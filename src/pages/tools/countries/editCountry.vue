<template>
  <div>
    <top-bar-card
      :newUrlName="`edit-countries`"
      :submit="updateCountry"
      :id="formData.id"
      :editUrlName="`edit-countries`"
      :navigationUrl="`countries`"
      :model="`country`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`countries`"
      :operation="`edit`"
      :listUrlName="`countries`"
    />
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updateCountry" id="country">
...
export default {
  ...
  methods: {
    async updateCountry() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("updateCountry", params);
        } catch (errors) {
    ...
    async getCountry() {
      const getCountry = await this.$store.dispatch("getCountry", {
        Id: this.$route.params.id,
      });
      this.formData = getCountry.data.data;
    },
